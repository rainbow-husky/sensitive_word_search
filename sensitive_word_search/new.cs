...
namespace sensitive_word_search
{
    ...
    public class TPL_sensitive_word_search
    {
        ...
        public ActionBlock<string> consoleBlock;

        public ActionBlock<string> fileBlock;

        public TPL_sensitive_word_search(string sensitive_words, string replace_words, string outputpath = "output.txt")
        {
            // 初始化函数
            ...
            consoleBlock = new ActionBlock<string>((input) =>
            {
                ...
                Log.Information($"字符串处理结果为：{input}");
            });
            ...
            fileBlock = new ActionBlock<string>((input) =>
            {
                try{
                    ...
                    { 
                        sw.WriteLine(input); 
                        Log.Information($"写入{outputpath}文件成功");
                    }
                }
                catch(Exception e)
                {
                    Log.Error($"写入{outputpath}文件失败");
                }
            ...
            {
                Console.WriteLine("can not link TPL dataflow blocks!");
                Log.Error("链接模块出错！");
            }
